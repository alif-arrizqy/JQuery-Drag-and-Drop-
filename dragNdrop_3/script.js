...
$(document).ready(function(){
	//yang memiliki class dragable bisa di pindahkan (drag)
	$(".dragableKelima").draggable({
		revert: true //kembali ke posisi awal
	});

	//class dropable dapat menerima hasil drag
	$(".dropableKelima").droppable({
		drop: function(event, ui){
			let nilaiBenarKelima = $(this).attr('title'); //nilai yg benar

			//mendapatkan nilai dari kotak yang dibawa
			// let tempatMenerima  = $(this).text();
			let kotakYangDibawaKelima = ui.draggable.text();

			if(kotakYangDibawaKelima == nilaiBenarKelima)
			{
				//hilangkan kotak yang dibawa (drag)
				ui.draggable.animate({'opacity' : '0'}, 100);

				$(this).removeClass("errorKelima"); //hapus class error

				//tambahkan class active dikotak yang menerima
				$('.ui-widget-headerKelima').addClass("activeKelima");

				//isi kotak yg dibawa dengan nilai benar
				$(this).text(nilaiBenarKelima);
				}else{
					$(this).addClass("errorKelima"); //tambahkan class error
			}
		} 
	});
});